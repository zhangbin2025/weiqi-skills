---
name: weiqi-fetcher
description: 多源围棋棋谱下载器 - 支持从OGS、野狐围棋等平台下载SGF棋谱
---

# weiqi-fetcher - 多源围棋棋谱下载器

从多种围棋网站/平台的分享链接自动下载SGF棋谱。
...
# 野狐对局
python3 main.py "https://h5.foxwq.com/yehunewshare/?chessid=1234567890123456789"

# 101围棋网
python3 main.py "https://www.101weiqi.com/play/p/14629903/"

# 弈客围棋（需要Playwright）
python3 main.py "https://home.yikeweiqi.com/mobile.html#/golive/room/12345/..."

# 元萝卜围棋
python3 main.py "https://jupiter.yuanluobo.com/robot-public/all-in-app/go/review?session_id=..."

# 星阵围棋
python3 main.py "https://m.19x19.com/app/dark/zh/sgf/70307160"

# 隐智智能棋盘
python3 main.py "http://app.izis.cn/web/#/live_detail?gameId=888522&type=2"

# 弈客少儿版
python3 main.py "https://shaoer.yikeweiqi.com/statichtml/game_analysis_mobile.html?p=..."

# 弈城围棋
python3 main.py "http://mobile.eweiqi.com/index_ZHCN.html?LNK=1&GNO=1478310481"

# 腾讯围棋
python3 main.py "https://h5.txwq.qq.com/txwqshare/index.html?chessid=1774531827010001676"

# 新博对弈
python3 main.py "https://weiqi.xinboduiyi.com/golive/index.html#/?gamekey=Local-20260322142755-51539"
```

### 指定输出文件

```bash
python3 main.py "https://online-go.com/game/85445564" -o ~/games/mygame.sgf
```

### 强制指定来源

```bash
python3 main.py "..." --source ogs
```
...
🌐 来源: ogs
🔗 URL: https://online-go.com/game/85445564
...
💾 文件保存: /tmp/weiqi_fetch/20260326_175713/ogs_85445564.sgf
...
- **实现方式**: 纯REST API
- **URL模式**: `https://jupiter.yuanluobo.com/robot-public/all-in-app/go/review?session_id=...`
- **特点**:
  - AI围棋机器人对局记录
  - 支持让子棋
  - 无需浏览器，速度最快（约0.3秒）
...
class NewsiteFetcher(BaseSourceFetcher):
    ...
    url_examples = ["https://newsite.com/game/12345"]
...
弈客围棋和星阵围棋使用浏览器自动化，需要安装 Playwright：
```bash
...
**原因**：坐标转换问题
**解决**：这是已知问题，正在修复中。请检查SGF文件中的坐标是否正确。
...
### v1.1.0 (2026-03-27)
- ✅ 新博对弈(xinboduiyi.com)支持
- ✅ Playwright 监听 WebSocket 获取实时棋谱
- ✅ 自定义坐标映射（纵/横双字母编码）
- ✅ 支持教室记谱分享链接

### v1.0.0 (2026-03-27)
- ✅ 腾讯围棋(txwq.qq.com)支持
- ✅ 通过 Playwright 监听 jsonp.php 响应
- ✅ 提取 JSONP 中的 chess 字段（转义SGF）
- ✅ 支持职业段位显示（P4段等）

### v0.9.0 (2026-03-27)
- ✅ 弈城围棋(eweiqi.com)支持
- ✅ 通过 gibo_load_data.php API 获取棋谱
- ✅ 自定义GIBO格式解析和SGF转换
- ✅ 支持弈城段位系统转换

### v0.8.0 (2026-03-27)
- ✅ 弈客少儿版(shaoer.yikeweiqi.com)支持
- ✅ 通过 yikemo/anon/ayalyse/init API 获取SGF
- ✅ Base64解码分享链接参数

### v0.7.0 (2026-03-27)
- ✅ 隐智智能棋盘(izis.cn)支持（Playwright + REST API）
- ✅ 解析隐智格式着法（+xxxx -xxxx）
- ✅ 支持国际赛事棋谱下载

### v0.6.0 (2026-03-27)
- ✅ 星阵围棋(19x19.com)支持（Playwright + localStorage）
- ✅ 从localStorage.engine提取棋谱数据
- ✅ 坐标自动转换（0-360索引 → SGF坐标）

### v0.5.0 (2026-03-26)
- ✅ 元萝卜围棋支持（Playwright浏览器自动化）
- ✅ AI围棋机器人对局下载
- ✅ 支持让子棋数据

### v0.4.0 (2026-03-26)
- ✅ 弈客围棋支持（Playwright浏览器自动化）
- ✅ 支持实时直播对局下载
- ✅ 自动提取完整SGF
