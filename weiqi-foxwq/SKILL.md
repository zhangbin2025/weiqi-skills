...
> 1. **网络连接** - 技能需要连接野狐围棋网站 (foxwq.com)
> 2. **协议解析** - 解析了野狐围棋的 Protobuf 二进制协议
> 3. **外部数据** - 从第三方网站提取公开棋谱数据
>
...
# 分享链接提取需要 Playwright（仅 WebSocket 模式需要）
...
💾 SGF已保存: /tmp/foxwq_123456789012345.sgf
...
### 数据安全
- 仅读取公开数据，不上传任何信息
- API 请求使用标准 HTTP/HTTPS
- WebSocket 仅用于获取公开对局数据
