...
【单个查询】
    python3 yichafen_fast.py 朱玲钰
    python3 yichafen_fast.py 熊益成

【批量查询 - 推荐】
    python3 yichafen_fast.py --batch 熊益成 田翔宇 朱玲钰
    python3 yichafen_fast.py --batch 宋夏 刘成龙
"""
...
def format_output(name, info, elapsed):
    ...
    if info.get('段位'):
        ...
        has_data = True
    if info.get('等级分'):
        print(f"📊 等级分: {info['等级分']}")
        has_data = True
    if info.get('总排名'):
        print(f"🏆 总排名: {info['总排名']}")
        has_data = True
    if info.get('省区排名'):
        print(f"📈 省区排名: {info['省区排名']}")
        has_data = True
    if info.get('本市排名'):
        print(f"📉 本市排名: {info['本市排名']}")
        has_data = True
    if info.get('省区'):
        print(f"📍 省区: {info['省区']}")
        has_data = True
    if info.get('城市'):
        print(f"🏙️ 城市: {info['城市']}")
        has_data = True
    if info.get('性别'):
        print(f"👤 性别: {info['性别']}")
        has_data = True
...
def format_batch_output(results, total_elapsed):
    ...
    for info in results:
        ...
        if info.get('总排名'):
            print(f"   🏆 总排名: {info['总排名']}")
        if info.get('省区'):
            print(f"   📍 {info.get('省区', '')} {info.get('城市', '')}")
        if not info.get('段位') and not info.get('等级分'):
            print(f"   ⚠️ 未找到段位注册信息")
    
    print("\n" + "="*60)
    print(f"⏱️  总耗时: {total_elapsed:.1f}秒 | 平均: {total_elapsed/len(results):.1f}秒/人")
    print("="*60)
    print(timer.format_report())
...
def main():
    if len(sys.argv) < 2:
        print(__doc__)
        print("\n【单个查询】")
        print("  python3 yichafen_fast.py <姓名>")
        print("  示例: python3 yichafen_fast.py 朱玲钰")
        print("\n【批量查询】")
        print("  python3 yichafen_fast.py --batch 姓名1 姓名2 姓名3 ...")
        print("  示例: python3 yichafen_fast.py --batch 熊益成 田翔宇 朱玲钰")
        sys.exit(1)
